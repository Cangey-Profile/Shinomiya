...
class ErisCommand(commands.Command):
...
class ErisGroup(commands.Group):
    """Overwrites the default Command to use permission levels,
    overwrites signature to hide aliases"""
    ...
    def command(self, *args: Any, **kwargs: Any) -> Callable:
        """Overwrites GroupMixin.command to use LyssCommand"""
...
def command(level: int, **kwargs: Any) -> Callable:
    ...
    return commands.command(cls=ErisCommand, **kwargs)


def group(level: int, **kwargs: Any) -> Callable:
    """Overwrites the default group to use LyssGroup"""
    kwargs['perm_level'] = level
    return commands.group(cls=ErisGroup, **kwargs)
