...
class Giveaways(commands.Cog):
    ...
    async def end_giveaway(self, giveaway: DBDict) -> None:
        ...
        try:
            ...
            await giveaway.channel.send(f"Congratulations! Here are the winners for `{description}` :tada:\n{fmt_winners}", allowed_mentions=AllowedMentions.all())
    ...
    async def setgiveaway(self, ctx: commands.Context, emoji: EmojiOrUnicode, channel: discord.TextChannel, role: str=None):
        ...
        Role can be @everyone, @here or none, this will be automatically pinged when you start a giveaway."""
        ...
        async with ctx.typing():
            ...
            if latest_giveaway:
                try:
                    ...
                    await ctx.send(f"Congratulations! Here are the **rerolled** winners for `{description}` :tada:\n{fmt_winners}", allowed_mentions=AllowedMentions.all())
            else:
